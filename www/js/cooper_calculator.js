...
var getRatingIndex = function (gender, age, distance) {
    a_range = ageRange(age);
    distance_ranges = cooper_norms[gender.toLowerCase()][a_range];
...
CooperCalculator.prototype.woman_cooper = function(obj) {
  ...
  var age = obj.age;
    if (distance > 0 && age > 0) {
      var ratingIndex = getRatingIndex('female', age, distance);
      setWomanMessage(obj,ratingIndex);
  }
};

CooperCalculator.prototype.man_cooper = function(obj) {
  var distance = obj.distance;
  var age = obj.age;
  if (distance > 0 && age > 0) {
    var ratingIndex = getRatingIndex('male', age, distance);
    setManMessage(obj,ratingIndex);
  }
};
...
function setWomanMessage (obj, value){
  if (value == 1) {
    obj.cooperMessage = "superwoman"
  }
  if (value == 2) {
  ...
  if (value == 5) {
    obj.cooperMessage = "in need to get in shape."
  }
}


function setManMessage (obj, value){
  if (value == 1) {
    obj.cooperMessage = "superman"
  }
  if (value == 2) {
    obj.cooperMessage = "above average"
  }
  ...
  if (value == 5) {
    obj.cooperMessage = "in serious need to get in shape."
  }
}
